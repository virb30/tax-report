...
describe('UpdateBrokerUseCase', () => {
  ...
  it('should throw an error if the broker is not found', async () => {
    const id = Uuid.create();
    await expect(() => useCase.execute({ id: id.value, name: '   ' })).rejects.toThrow(
  ...
  it('should throw an error when CNPJ is duplicated by another broker', async () => {
    ...
    await expect(
      useCase.execute({ id: existingBroker.id.value, cnpj: otherBroker.cnpj.value }),
    ).rejects.toThrow(new Error('CNPJ ja cadastrado para outra corretora.'));
  });
  ...
  it('update does not fail when CNPJ is same as own broker', async () => {
    ...
    const result = await useCase.execute({
      id: existingBroker.id.value,
      cnpj: existingBroker.cnpj.value,
      name: 'XP Investimentos',
    });
  ...
  it('should throw an error when code is duplicated by another broker', async () => {
    ...
    await expect(
      useCase.execute({ id: existingBroker.id.value, code: otherBroker.code }),
    ).rejects.toThrow(new Error('Código ja cadastrado para outra corretora.'));
  });
