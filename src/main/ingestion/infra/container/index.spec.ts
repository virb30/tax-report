...
const showOpenDialog = jest.mocked(dialog.showOpenDialog);
...
describe('createIngestionModule', () => {
  ...
  it('can be instantiated from shared infrastructure plus portfolio exports and serve import IPC contracts', async () => {
    mockBrokerRepository.findAll.mockResolvedValue([
      Broker.restore({
    ...
    const handler = handlers.get(listDailyBrokerTaxesContract.channel);
  ...
  it('registers import IPC channels directly from the module and serves file selection', async () => {
    showOpenDialog.mockResolvedValue({
      canceled: false,
      filePaths: ['C:/imports/operations.csv'],
    });
    const handlers = new Map<string, IpcHandler>();
    const ipcMain: IpcMainHandleRegistry = {
      handle: (channel, listener) => {
    ...
    expect([...handlers.keys()].sort()).toEqual(
      importIpcContracts.map((contract) => contract.channel).sort(),
    );
    await expect(
      handlers.get(importSelectFileContract.channel)?.({} as Electron.IpcMainInvokeEvent),
    ).resolves.toEqual({
