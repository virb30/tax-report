...
describe('CreateBrokerUseCase', () => {
  ...
  it('should throw an error when name is empty', async () => {
    await expect(useCase.execute({ name: '   ', code: 'XP', cnpj: '12.345.678/0001-90' })).rejects.toThrow(new Error('Nome da corretora e obrigatorio.'));
  ...
  it('should throw an error when CNPJ is duplicated', async () => {
    ...
    await expect(useCase.execute({ name: 'Outra Corretora', code: 'OUTRA', cnpj: '02.332.886/0001-04' })).rejects.toThrow(new Error('CNPJ já cadastrado para outra corretora.'));
  });
});
