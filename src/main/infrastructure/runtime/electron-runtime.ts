...
export class ElectronRuntime implements Runtime {
  ...
  private render(browserWindow: BrowserWindow) {
    ...
    void browserWindow.loadFile(path.join(this.currentDirectory, `../renderer/${this.appName}/index.html`));
  }
  ...
  private setDevServerUrl(devServerUrl?: string) {
    this.devServerUrl = devServerUrl ?? MAIN_WINDOW_VITE_DEV_SERVER_URL ?? process.env.MAIN_WINDOW_VITE_DEV_SERVER_URL;
  }
