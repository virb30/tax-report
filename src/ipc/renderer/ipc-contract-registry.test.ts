...
describe('ipc contract registry', () => {
  ...
  it('rejects duplicate IPC channels', () => {
    const [contract] = ipcContracts;
    ...
    ).toThrow('Duplicate IPC contract channel');
  });
  ...
  it('rejects duplicate renderer API names for exposed contracts', () => {
    const [contract] = rendererExposedIpcContracts;
    ...
    expect(() =>
      assertUniqueIpcContractMetadata([
        ...
        {
          ...contract,
          id: 'test.duplicateApiName',
          channel: 'test:duplicate-api-name',
        },
      ]),
    ).toThrow('Duplicate IPC contract renderer API name');
  });
