...
describe('database', () => {
  ...
  it('returns the backend-configured sqlite database path', () => {
    const databasePath = '/tmp/tax-report/backend.sqlite';

    const resolvedPath = getDatabasePath({ sqlitePath: databasePath });

    expect(resolvedPath).toBe(databasePath);
  });

  it('creates backend runtime config from environment without retired runtime APIs', () => {
    const config = createBackendRuntimeConfig(
      { DATABASE_PATH: './storage/app.sqlite' },
      '/srv/tax-report/backend',
    );

    expect(config.database.sqlitePath).toBe('/srv/tax-report/backend/storage/app.sqlite');
  });

  it('rejects backend runtime config without a database path', () => {
  ...
  it('creates and initializes a file database', async () => {
    ...
    const sqlitePath = path.join(tmpDirectory, 'configured.sqlite');
  ...
  it('creates missing parent directories before initializing a file database', async () => {
    const tmpDirectory = fs.mkdtempSync(path.join(os.tmpdir(), 'tax-report-nested-'));
    const sqlitePath = path.join(tmpDirectory, 'storage', 'sqlite', 'configured.sqlite');
    const { database: createdDatabase, databasePath } = await createAndInitializeDatabase(
      { sqlitePath },
      false,
    );
  ...
  it('creates file database using default runSeeds option', async () => {
    ...
    const { database: createdDatabase } = await createAndInitializeDatabase({
      sqlitePath: path.join(tmpDirectory, 'default-option.sqlite'),
    });
    database = createdDatabase;
