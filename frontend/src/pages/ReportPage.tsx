...
import { ReportItemStatus, type AssetType } from '@/types/api.types';
...
export function ReportPage(): JSX.Element {
  ...
  const handleGenerateReport = useCallback(async () => {
    ...
    setFeedbackMessage('');
    try {
      const result = await getTaxReportApi().generateAssetsReport({
  ...
  const handleRepair = async (ticker: string) => {
    try {
      const result = await getTaxReportApi().listAssets({ pendingOnly: false });
  ...
  const saveRepair = async (
    ...
    try {
      if (data.assetType && data.assetType !== editingAsset?.assetType) {
        await getTaxReportApi().repairAssetType({ ticker, assetType: data.assetType });
      }

      const updateResult = await getTaxReportApi().updateAsset({ ticker, ...data });
