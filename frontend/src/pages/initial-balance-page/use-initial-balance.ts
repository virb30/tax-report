...
import type { InitialBalanceDocument, PositionListItem } from '@/types/api.types';
...
export function useInitialBalance() {
  ...
  async function loadDocuments(targetYear = year): Promise<void> {
    ...
    try {
      const result = await getTaxReportApi().listInitialBalanceDocuments({ year: targetYear });
      setDocuments(unwrapApiResult(result).items);
    } catch (error: unknown) {
      setErrorMessage(buildErrorMessage(error));
    } finally {
  ...
  async function loadPositions(targetYear = year): Promise<void> {
    ...
    try {
      const result = await getTaxReportApi().listPositions({ baseYear: targetYear });
  ...
  async function saveInitialBalance(): Promise<void> {
    ...
    try {
      unwrapApiResult(
        await getTaxReportApi().saveInitialBalanceDocument({
  ...
  async function deleteDocument(document: InitialBalanceDocument): Promise<void> {
    ...
    try {
      const result = await getTaxReportApi().deleteInitialBalanceDocument({
