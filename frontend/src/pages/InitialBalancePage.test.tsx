...
import type { InitialBalanceDocument, ListInitialBalanceDocumentsResult } from '@/types/api.types';
import type { ListPositionsResult, PositionListItem } from '@/types/api.types';
...
describe('InitialBalancePage', () => {
  ...
  beforeEach(() => {
    ...
    taxReportApi.listInitialBalanceDocuments.mockResolvedValue(createDocumentsResult([]));
    taxReportApi.listPositions.mockResolvedValue(createPositionsResult([]));
    taxReportApi.saveInitialBalanceDocument.mockResolvedValue({
      ok: true,
      data: createDocument(),
    });
    taxReportApi.deleteInitialBalanceDocument.mockResolvedValue({
      ok: true,
      data: { deleted: true },
    });
  ...
  it('loads brokers, saved documents, and positions on mount', async () => {
    taxReportApi.listInitialBalanceDocuments.mockResolvedValueOnce(
      createDocumentsResult([createDocument()]),
    );
    taxReportApi.listPositions.mockResolvedValueOnce(
    ...
    expect(taxReportApi.listInitialBalanceDocuments).toHaveBeenCalledWith({ year: 2025 });
    expect(taxReportApi.listPositions).toHaveBeenCalledWith({ baseYear: 2025 });
  });
  ...
  it('saves multiple allocations, refreshes documents, and clears the editor', async () => {
    ...
    expect(taxReportApi.saveInitialBalanceDocument).toHaveBeenCalledWith({
      ticker: 'IVVB11',
      year: 2025,
      assetType: AssetType.Stock,
      name: 'iShares Core S&P 500',
  ...
  it('loads an existing document for editing and replaces it on save', async () => {
    ...
    taxReportApi.listPositions
      .mockResolvedValueOnce(createPositionsResult([]))
      .mockResolvedValueOnce(createPositionsResult([]));
    taxReportApi.saveInitialBalanceDocument.mockResolvedValueOnce({
      ok: true,
  ...
  it('deletes a saved document and removes it from the table after refresh', async () => {
    ...
    expect(taxReportApi.deleteInitialBalanceDocument).toHaveBeenCalledWith({
      ticker: 'IVVB11',
      year: 2025,
    });
    expect(
  ...
  it('rejects missing broker or quantity issues and keeps positions separated from documents', async () => {
    ...
    expect(taxReportApi.saveInitialBalanceDocument).not.toHaveBeenCalled();
    expect(screen.getByText('Documentos salvos de saldo inicial')).toBeInTheDocument();
    expect(screen.getByText('Posições em 31/12/2025')).toBeInTheDocument();
    expect(
      screen.getByText(
