...
import type { DailyBrokerTaxItem } from '@/types/api.types';
...
export function useDailyBrokerTaxes(brokers: Broker[]) {
  ...
  const loadTaxes = useCallback(async (): Promise<void> => {
    setIsLoading(true);
    try {
      const result = await getTaxReportApi().listDailyBrokerTaxes();
  ...
  async function saveTax(): Promise<void> {
    ...
    setErrorMessage('');
    try {
      const result = await getTaxReportApi().saveDailyBrokerTax({
        date: form.date,
  ...
  async function importTaxes(file: File | null): Promise<void> {
    if (!file) {
      return;
    }
    ...
    setErrorMessage('');
    try {
      const result = await getTaxReportApi().importDailyBrokerTaxes({
        file,
      });
      setFeedbackMessage(
  ...
  async function deleteTax(tax: DailyBrokerTaxItem): Promise<void> {
    ...
    setErrorMessage('');
    try {
      const result = await getTaxReportApi().deleteDailyBrokerTax({
        date: tax.date,
